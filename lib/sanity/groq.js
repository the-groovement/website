...
*[_type == "event" && startTime >= "${currentTime}"] | order(startTime asc) [$start...$end] {
  ...,
  venue-> {
    ...,
    intro[],
...
{
  "featuredArtists": *[_type == "post" && featured == true && "artists" in categories[]->slug.current] | order(publishedAt desc) [0...3] {
    ...,
    authors[]->,
    categories[]->
  },
  "featuredVenue": *[_type == "venue" && featured == true] {
    ...,
    intro[],
    history[],
    ada[],
  ...
  "events": *[_type == "event" && startTime >= "${currentTime}" && startTime <= "${new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString()}" && featured == true] | order(startTime asc) [0...10] {
    ...,
    venue-> {
      ...,
      intro[],
...
export const eventsSearchQueryNoEndTime = groq`
  *[_type == "event" && (
    ...
    lineup match $searchInput
  ) && $startTime <= startTime] | order(startTime asc) [$start...$end] {
    ...,
    venue-> {
      ...,
      intro[],
...
export const eventsSearchQuery = groq`
  *[_type == "event" && (
    eventName match $searchInput ||
    venue->name match $searchInput
  ) && startTime <= $endTime && $startTime <= startTime] | order(startTime asc) [$start...$end] {
    ...,
    venue-> {
      ...,
      intro[],
...
*[
  (_type == "post" && $categoryId in categories[]->slug.current) ||
  (_type == "venue" && $categoryId == "venues")
...
export const postsCategorySearch = groq`
  *[
    ($categoryId == "venues" && _type == "venue" && name match $searchInput) ||
    (_type == "post" && title match $searchInput && $categoryId in categories[]->slug.current)
  ] | order(publishedAt desc) [$start...$end] {
    ...,
     _type == "venue" => {
      intro,
...
export const postsSearch = groq`
  *[_type == "post" && title match $searchInput] | order(publishedAt desc) [$start...$end] {
    ...,
    authors[]->,
    photographer->,
    categories[]->
